...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 2 + i, Mouse.y + 2 + i, 1, 14 - (i * 2), Color(32, 80, 31, 255));
		Render::Clear(Mouse.x + 6, Mouse.y + 6, 1, 8, Color(32, 80, 31, 255));
		Render::Clear(Mouse.x + 7, Mouse.y + 7, 1, 9, Color(32, 80, 31, 255));
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 8 + i, Mouse.y + 8 + i, 1, 4 - i, Color(32, 80, 31, 255));
		Render::Clear(Mouse.x + 8, Mouse.y + 14, 1, 4, Color(32, 80, 31, 255));
		Render::Clear(Mouse.x + 9, Mouse.y + 16, 1, 2, Color(32, 80, 31, 255));
	}
}
