...
CNETVAR_FUNC(unsigned char, GetLifeState, 0xD795CCFC); //m_lifeState
	...
	CNETVAR_FUNC(float, ForceBone, 0x267C); // m_nForceBone 
	...
	{
		return (float*)((uintptr_t)this + 0x39F8);
	}
	QAngle* GetEyeAnglesPointer()
	{
		return reinterpret_cast<QAngle*>((DWORD)this + (DWORD)0xAA08);
	}
	Vector* EyeAnglePointer()
	{
		return (Vector*)((uintptr_t)this + 0x0000AA08);
	}
	QAngle GetEyeAngles()
	{
		return *reinterpret_cast<QAngle*>((DWORD)this + (DWORD)0xAA08);
	...
	float GetMaxSpeed()
	{
		return *(float*)((DWORD)this + GET_NETVAR("DT_BasePlayer", "m_flMaxspeed"));
	}
	...
	Vector GetPredicted(Vector p0, IClientEntity * ent);
};